...
*P.S.: Сейчас используется 32 бита для хранения времени. Стоит подумать над выделением 64 бит (19-2038 problem)*

*P.P.S.: Комментарий по авторским фишкам контракта. Я решил их не включать, поскольку это сильно выбивается из ТЗ. Но общая идея такая: можно создать пополняемый вестинг. Сделать это несложно. Пусть доступная сумма к выплате описывается функцией ```f(args, t)```, где ```args``` - параметры вестинга, ```t``` - текущее время. При пополнении вестинга ```args'``` (новые параметры) функция выплат обновляется как ```g(args, args', t) f(args, t) + (1 - g(args, args', t)) f(args', t)```,
где ```g(args, args', t)``` весовая функция ```из отрезка [0, 1]```. Упрощенно это можно понимать так: остаточная выплата по первому вестигу - прямая, новый вестинг - тоже прямая. Из них можно получить новую прямую (то есть график выплат), взяв их взвешенную сумму.*
